...
class CANCERDataColl:
    def __init__(self,batch_size=128):
        ...
        train_set = torchvision.datasets.ImageFolder(os.path.join('/Users/Ayomide_1/Downloads/archive (1)',
                                                                          'train'),transform=tfs)
        val_set = torchvision.datasets.ImageFolder(os.path.join('/Users/Ayomide_1/Downloads/archive (1)',
                                                                          'test'),transform=tfs_1)
        self.train_loader = torch.utils.data.DataLoader(train_set, batch_size=batch_size, shuffle=True, num_workers=4)
        
        self.test_loader = torch.utils.data.DataLoader(val_set, batch_size=batch_size, shuffle=False, num_workers=4)
...
class FairFaceDataset():
    def __init__(self, data, path , batch_size=1, transform = None):
        super().__init__()
...
class TRAITDataColl():
    def __init__(self,batch_size=1):
        ...
        full_train_set = FairFaceDataset(df,
                                    "/Users/Ayomide_1/ml/Branchy_side_channels/earlyexitnet",
                                     transform=self.tfs
                                    )
        #full testing set
        full_test_set = FairFaceDataset(df2,
                                    "/Users/Ayomide_1/ml/Branchy_side_channels/earlyexitnet",
                                     transform=self.tfs_1
                                    )
        self.train_loader = torch.utils.data.DataLoader(full_train_set, batch_size=batch_size, shuffle=True, num_workers=4)
        
        self.test_loader = torch.utils.data.DataLoader(full_test_set, batch_size=batch_size, shuffle=False, num_workers=4)        
