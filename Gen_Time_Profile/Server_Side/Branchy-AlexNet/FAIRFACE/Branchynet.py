...
class B_Lenet(nn.Module):
    def __init__(self, exit_threshold=[0.0001, 0.05,10000]):
        ...
        self.exit_loss_weights = [1.0,1.0,0.8] #weighting for each exit when summing loss
...
class B_AlexNet(B_Lenet):
    def _build_backbone(self):
        strt_bl = ConvPoolAc(3, 32, kernel=5, stride=1, padding=2)
        self.backbone.append(strt_bl)

        #adding ConvPoolAc blocks - remaining backbone
        mid_layer = ConvPoolAc(32, 64, kernel=5, stride=1, padding=2)
        self.backbone.append(mid_layer)
        bb_layers = []
        bb_layers.append(ConvPoolAc(64, 96, kernel=3, stride=1, padding=1) )
        bb_layers.append(ConvPoolAc(96, 96, kernel=3, stride=1, padding=1) )
        bb_layers.append(ConvPoolAc(96, 64, kernel=3, stride=1, padding=1) )
        bb_layers.append(nn.Flatten())
        bb_layers.append(nn.Linear(3136, 256, bias=False))
    ...
    def _build_exits(self):
        #early exit 1
        ee1 = nn.Sequential(
            ConvPoolAc(32, 64, kernel=3, stride=1, padding=2),
            ConvPoolAc(64, 32, kernel=3, stride=1, padding=1),
            nn.Flatten(),
            nn.Linear(25088,3, bias=False),
            )
        self.exits.append(ee1)
