    ...
    def __init__(self, chanIn, chanOut, kernel=3, stride=1, padding=1, p_ceil_mode=False,bias=True):
        ...
        self.layer = nn.Sequential(
            nn.Conv2d(chanIn, chanOut, kernel_size=kernel,
                ...
                nn.ReLU(True),
            nn.MaxPool2d(3, stride=2, ceil_mode=p_ceil_mode),
             #ksize, stride
...
class B_Lenet(nn.Module):
    def __init__(self, exit_threshold=[0.025]):
        ...
        self.exit_loss_weights = [1.0,0.8,0.5] #weighting for each exit when summing loss
...
class B_AlexNet(B_Lenet):
    def _build_backbone(self):
        ...
        bb_layers.append(nn.Flatten())
        bb_layers.append(nn.Linear(1024, 256, bias=False))
        bb_layers.append(m)
