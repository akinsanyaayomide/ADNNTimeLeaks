...
class RANet(nn.Module):
    def __init__(self, args,exit_threshold=[ 9.9942e-01,  9.9773e-01,  9.9245e-01,  9.5594e-01,  8.2895e-01,
        -1.0000e+08]):
        ...
        self.exits_track ={'exit_1':0,'exit_2':0,'exit_3':0,'exit_4':0,'exit_5':0,'exit_6':0}
        ...
        for ii in range(self.nScales):
            ...
            for i in range(self.nBlocks[ii+1]):
                ...
                if n_block_curr > self.nBlocks[ii]:
                    ...
                    elif args.data.startswith('cancer'):
                        self.classifier.append(self._build_classifier_imagenet(nIn, 2))
    ...
    def exit_criterion_top1(self, x,index): #NOT for batch size > 1
        #evaluate the exit criterion on the result provided
        #return true if it can exit, false if it can't
        with torch.no_grad():
            pk = nn.functional.softmax(x, dim=1)#nn.Softmax(dim=1).cuda()
            top1 = torch.max(pk) #x)
            #print('pred->',pk.argmax(dim=1),'top1->',top1, 'exit threshold->', self.exit_threshold)
            return top1 > self.exit_threshold[0][index]
        ...
        for ii in range(self.nScales):
            ...
            for i in range(self.nBlocks[ii+1]):
                ...
                if self.scale_flows[ii][i]._blockType() == 'norm':
                    ...
                    #print('n_block_curr before->',n_block_curr)
                    n_block_curr += 1
                    #print('n_block_curr before->',n_block_curr)
                    if n_block_curr > self.nBlocks[ii]:
                        #print('classifier_idx->',classifier_idx)
                        res = self.classifier[classifier_idx](_x)
                        classifier_idx += 1
                        #print('done')
                        if self.exit_criterion_top1(res,classifier_idx-1):
                            b ='exit_'+str(classifier_idx)
                            self.exits_track[b] +=1
                            return res,classifier_idx
